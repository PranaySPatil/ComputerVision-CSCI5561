...
def find_match(img1, img2):
    ...
    for i in range(len(distances)):
        if distances[i][0] < 0.75 * distances[i][1]:
            x2_one_to_two.append(kp2[i].pt)
            x1_one_to_two.append(kp1[indices[i][0]].pt)

    nbrs = NearestNeighbors(n_neighbors=2).fit(des2)
    distances, indices = nbrs.kneighbors(des1, 2)
    x1_two_to_one = []
    x2_two_to_one = []
    for i in range(len(distances)):
        if distances[i][0] < 0.75 * distances[i][1]:
            x2_two_to_one.append(kp1[i].pt)
            x1_two_to_one.append(kp2[indices[i][0]].pt)
    
    x1=[]
    x2=[]
    for i in range(len(x1_one_to_two)):
        for j in range(len(x2_two_to_one)):
            if x1_one_to_two[i] == x2_two_to_one[j]\
                     and x2_one_to_two[i] == x1_two_to_one[j]:
                x1.append(x1_one_to_two[i])
                x2.append(x2_one_to_two[i])
                break
...
def compute_F(pts1, pts2):
    ...
    for iteration in range(ransac_iter):
        ...
        for i in range(number_of_random_samples):
            sample_index = random.randint(0, pts1.shape[0]-1)
            ux, uy = pts1[sample_index]
            vx, vy = pts2[sample_index]
        ...
        for i in range(pts1.shape[0]):
            u_test = np.array([pts1[i][0], pts1[i][1], 1]).reshape(3, 1)
            v_test = np.array([pts2[i][0], pts2[i][1], 1]).reshape(1, 3)
            error = np.abs(np.dot(v_test, np.dot(F, u_test)))
            if error < ransac_thr:
                inliers_count += 1
                inlier_x1.append(pts1[i])
                inlier_x2.append(pts2[i])
...
def triangulation(P1, P2, pts1, pts2):
    ...
    for i in range(len(pts1)):
        u_skew = np.array([[0, -1, pts1[i][1]], [1, 0, -1*pts1[i][0]], [-1*pts1[i][1], pts1[i][0], 0]])
        v_skew = np.array([[0, -1, pts2[i][1]], [1, 0, -1*pts2[i][0]], [-1*pts2[i][1], pts2[i][0], 0]])
        u_skew_cross_P1 = np.dot(u_skew, P1)
        v_skew_cross_P2 = np.dot(v_skew, P2)
        A = np.array([u_skew_cross_P1[0], u_skew_cross_P1[1], v_skew_cross_P2[0], v_skew_cross_P2[1]])
...
def disambiguate_pose(Rs, Cs, pts3Ds):
    ...
    for i in range(len(Rs)):
        ...
        inliers = []
        for pt in pts3Ds[i]:
            z = np.dot(Rs[i][2], np.array(pt).reshape(3,1)-Cs[i])
            if z[0] > 0:
                inliers.append(pt)
                n += 1
        
        print(str(i+1) + " inliers - " + str(n))
        if n > max_inliers:
            pts3D = inliers
            max_inliers = n
            R = Rs[i]
...
def compute_rectification(K, R, C):
    ...
    rz_bar = np.array([[0], [0], [1]])
    rz = (rz_bar - (np.dot(rz_bar.T, rx)*rx)) / np.linalg.norm(rz_bar - (np.dot(rz_bar.T, rx)*rx))
    ry = np.cross(rz, rx, axis=0).reshape(3, 1)
    R_rect = np.array([[rx.T], [ry.T], [rz.T]]).reshape(3, 3)

    K_inv = np.linalg.inv(K)

    H1 = np.dot(K, np.dot(R_rect, K_inv))
    H2 = np.dot(K, np.dot(R_rect, np.dot( R.T, K_inv)))
...
def dense_match(img1, img2):
    ...
    kps = [cv2.KeyPoint(x, y, 3) for y in range(0, img1.shape[0]) for x in range(0, img1.shape[1])]
    kp1, img1_descriptors = sift.compute(img1, kps)
    kp2, img2_descriptors = sift.compute(img2, kps)
    img1_descriptors = img1_descriptors.reshape((img1.shape[0], img1.shape[1], 128))
    img2_descriptors = img2_descriptors.reshape((img2.shape[0], img2.shape[1], 128))

    for x in range(0, img1.shape[0]):
        for y in range(1, img1.shape[1]):
            disparity_map = np.zeros((1, y))
            for y_ in range(0, y):
                cost = np.subtract(img1_descriptors[x, y, :], img2_descriptors[x, y_, :])
                disparity_map[0, y_] = np.linalg.norm(cost) 
           
            min_cost = np.unravel_index(np.argmin(disparity_map, axis=None), disparity_map.shape)
            disparity[x, y] = y - min_cost[1]
